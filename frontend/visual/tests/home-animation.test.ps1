...
Assert-NotContains $index '<a href="#events">Events</a>' "Menu should not keep the redundant Events quick link."
Assert-Contains $index 'data-modal-target="signupModal"' "Sign up button should open the signup modal."
Assert-Contains $index 'data-modal-target="loginModal"' "Login button should open the login modal."
Assert-Contains $index 'id="loginModal"' "Login modal should exist."
Assert-Contains $index 'id="signupModal"' "Signup modal should exist."
Assert-Contains $index 'exploreMenu.removeAttribute("open")' "Explore toggle should clear the details open state."
Assert-NotContains $css 'nav-dropdown:focus-within' "Explore menu should not keep active styling after the toggle is closed."
Assert-NotContains $css 'nav-dropdown:hover .nav-dropdown-menu' "Explore dropdown should not stay open from hover after the toggle closes."
Assert-Contains $css '.account-modal' "Account modals should have dialog styling."
Assert-Contains $index 'event.preventDefault();' "Account modal forms should not reload the page."
